...
<? foreach($list as $post): ?>
	<item>
		...
		<dsq:thread_identifier>blog_post_<?= $post->id ?></dsq:thread_identifier>
...
>
<?
	$pages = Wiki_Page::create()->find_all();
	$site_url = site_url('/', true);
?>
<channel>
	<title>Site Title</title>
	...
	<item>
		...
		<link><?= $site_url ?>docs/<?= $page->url_title ?>/</link>
		<pubDate><?= date("D, j M Y G:i:s", strtotime($page->published_date)) ?> +0000</pubDate>
		<dc:creator><![CDATA[<?= Users_User::create()->find($page->created_user_id)->login ?>]] > </dc:creator>
		<category>uncategorized</category>
		<category domain="category" nicename="uncategorized">Uncategorized</category>
		<guid isPermaLink="false"><?= $site_url ?>docs/<?= $page->url_title ?>/</guid>
		<description></description>
		<content:encoded><![CDATA[<?= $page->content ?>]] > </content:encoded>
		<excerpt:encoded><![CDATA[]] > </excerpt:encoded>
		<dsq:thread_identifier>wiki_page_<?= $page->id ?></dsq:thread_identifier>
		<wp:post_id><?= $page->id ?></wp:post_id>
		<wp:post_date><?= date("D, j M Y G:i:s", strtotime($page->created_at)) ?> +0000</wp:post_date>
		<wp:post_date_gmt><?= date("D, j M Y G:i:s", strtotime($page->created_at)) ?> +0000</wp:post_date_gmt>
		<wp:comment_status>open</wp:comment_status>
		<wp:ping_status>open</wp:ping_status>
		<wp:post_name><?= $page->url_title ?></wp:post_name>
		<wp:status>publish</wp:status>
		<wp:post_parent>0</wp:post_parent>
		<wp:menu_order>0</wp:menu_order>
		<wp:post_type>post</wp:post_type>
